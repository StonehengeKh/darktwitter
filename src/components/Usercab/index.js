...
class UserCab extends Component {
  ...
  change = () =>
    this.setState(prevState => ({ ...prevState, change: !prevState.change }));
  userSave = () => {
  ...
  render() {
    ...
    return (
      <div>
        <div>
          <button className="button" onClick={setUserToUpdate}>
            Change user info
          </button>
        </div>
        {update && <p>Loading...</p>}
        ...
        {updateUs ? (
          <>
            ...
            {touch &&
            updateUs.login.length > 3 &&
            updateUs.email.includes("@") &&
            updateUs.password.length > 6 &&
            updateUs.password === updateUs.confirmPassword ? (
              <button className="button" onClick={this.userSave}>
                Save
              </button>
            ) : null}
          </>
